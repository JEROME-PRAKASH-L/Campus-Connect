...
import { dayName, shortDate } from '@/lib/utilities/format';
import { useShell } from '@/components/layout/AppShell';
import { PageHeader, PageState } from '@/components/ui/PageHeader';
import { KpiCards } from '@/components/ui/primitives';
import { DataTable, Figure, TwoLine } from '@/components/tables/DataTable';
import { can } from '@/config/permissions';
import { InternalMarksEditor } from './components/InternalMarksEditor';
...
export const Examinations = () => {
  ...
  const daysToFirst = first ? Math.max(0, Math.ceil((+new Date(first.date) - now) / 86_400_000)) : 0;
  ...
  return (
    <>
      ...
      {staff && can(user.role, 'marks:write') ? (
        <InternalMarksEditor
          subjects={[...new Map(data.exams.map((e) => [e.subjectId, { value: e.subjectId, label: `${e.code} · ${e.name}` }])).values()]}
        />
      ) : null}
    </>
  );
