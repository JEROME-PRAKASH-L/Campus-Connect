...
export const Settings = ({ theme, onTheme }: { theme: string; onTheme: (t: string) => void }) => {
  ...
  if (user.role === 'ADMIN') {
    groups.push({
      ...
      rows: [
        { label: 'Student', sub: 'Read own record · submit work', control: <span style={{ fontSize: 12.5, opacity: 0.7 }}>12 permissions</span> },
        { label: 'Faculty', sub: 'Registers, marks, materials, approvals', control: <span style={{ fontSize: 12.5, opacity: 0.7 }}>24 permissions</span> },
        { label: 'HOD', sub: 'Department-wide read & approvals', control: <span style={{ fontSize: 12.5, opacity: 0.7 }}>31 permissions</span> },
        { label: 'Administrator', sub: 'Full access including finance', control: <span style={{ fontSize: 12.5, opacity: 0.7 }}>All permissions</span> },
        { label: 'Parent', sub: 'Read-only on the linked ward', control: <span style={{ fontSize: 12.5, opacity: 0.7 }}>7 permissions</span> },
      ],
    });
  }
